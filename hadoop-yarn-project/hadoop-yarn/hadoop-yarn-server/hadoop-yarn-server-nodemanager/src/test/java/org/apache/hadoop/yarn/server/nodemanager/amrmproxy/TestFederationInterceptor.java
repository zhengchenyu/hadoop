...
public class TestFederationInterceptor extends BaseAMRMProxyTest {
  ...
  @Test
  public void testDownWithKillUnManagedApplication() throws IOException, InterruptedException {
    UserGroupInformation ugi =
        interceptor.getUGIWithToken(interceptor.getAttemptId());
    ugi.doAs((PrivilegedExceptionAction<Object>) () -> {
      // Register the application
      ...
      GetApplicationsResponse response = homeRM.getApplications(request);
      ...
      Assert.assertEquals(3, secondaries.size());
      for (MockResourceManagerFacade rm : secondaries.values()) {
        response = rm.getApplications(request);
        Assert.assertEquals(0, response.getApplicationList().size());
      }
      return null;
    });
