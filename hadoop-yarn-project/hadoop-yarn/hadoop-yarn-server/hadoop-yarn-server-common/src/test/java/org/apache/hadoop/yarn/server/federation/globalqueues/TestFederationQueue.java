...
public class TestFederationQueue {
  ...
  public void testParseQueue() throws Exception {
    String queueJson = GlobalQueueTestUtil.loadFile("globalqueues/tree-queue.json");
    FederationQueue fq = GlobalQueueTestUtil.parseQueue(queueJson);
    fq.validate();
  ...
  public void testMergeFedQueue() throws Exception {
    List<FederationQueue> toMerge = GlobalQueueTestUtil.generateFedCluster();
  ...
  public void testPropagateFedQueue() throws Exception {
    ...
    String queueJson = GlobalQueueTestUtil.loadFile("globalqueues/tree-queue-adaptable.json");
    ...
    for (int i = 0; i < numSubclusters; i++) {
      queueJson = String.format(queueJson, "A1", GlobalQueueTestUtil.toJSONString(guar),
          GlobalQueueTestUtil.toJSONString(max), GlobalQueueTestUtil.toJSONString(used), GlobalQueueTestUtil.toJSONString(dem));
      FederationQueue temp = GlobalQueueTestUtil.parseQueue(queueJson);
